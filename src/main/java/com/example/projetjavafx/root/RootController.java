...
public class RootController {
    ...
    protected void onMessagerieClick(ActionEvent event) {
        if (SessionManager.getInstance().getCurrentUserId()==-1){
            ...
            loadView("/com/example/projetjavafx/messagerie/discussion/discussion.fxml", event);
        }
    }
    @FXML
    private void onNotificationsClick(ActionEvent event) {
        if (SessionManager.getInstance().getCurrentUserId() == -1) {
            loadView("/com/example/projetjavafx/auth/login-view.fxml", event);
        } else {
            loadView("/com/example/projetjavafx/notifications/notifications.fxml", event);
        }
    }
    @FXML
